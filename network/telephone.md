...
* 2G, 3G への発展
    ...
    * 基地局アンテナ
        * 空中の高い場所を見つけることが重要。
            * 一部の通信事業者はローマカトリック教会との提携を築くようになった
                * ローマカトリック教会は世界中に多数の高貴なアンテナ用の場所を所有していて、すべてが統一的に管理されている。


                

いつでも、各携帯電話は論理的に1つの特定のセルにあり、そのセルの基地局の制御下にあります。携帯電話が物理的にセルを離れると、その基地局は電話の信号が消えていくのに気づき、周囲のすべての基地局に電話からどれだけの電力が供給されているかを尋ねます。応答が返されると、基地局は所有権をセルに移し、最も強い信号を取得します。ほとんどの条件下で、それは電話が現在配置されているセルです。次に、電話に新しい上司が通知され、通話が進行中の場合は、新しいチャネルに切り替えるように求められます（古いチャネルは隣接するセルで再利用されないため）。ハンドオフと呼ばれるこのプロセスには、約300ミリ秒かかります。チャネルの割り当ては、システムの神経の中心であるMSCによって行われます。基地局は実際には単なるダム無線リレーです。